...
def accomplishment_orm_to_domain(orm: AccomplishmentORM )->Accomplishment:
    ...
    domain:Accomplishment = Accomplishment(orm.name, orm.difficulty, orm.description, orm.status,attributes, professions,titles,missions)
    domain.id=orm.id
    return domain
...
def view_owned_accomplishments(search:str | None=None) -> list[Accomplishment]:
    if not search:
        accomplishments:Any = session.query(AccomplishmentORM).filter(AccomplishmentORM.status == "unlocked").all()
        domain_accomplishments = [accomplishment_orm_to_domain(orm) for orm in accomplishments] 
        return domain_accomplishments
    else:
        accomplishments:Any = session.query(AccomplishmentORM).filter(
            AccomplishmentORM.status == "unlocked").filter(
                or_(
...
def view_unowned_accomplishments(search:str | None=None) -> list[Accomplishment]:
    if not search:
        accomplishments:Any = session.query(AccomplishmentORM).filter(AccomplishmentORM.status == "locked").all()
        domain_accomplishments = [accomplishment_orm_to_domain(orm) for orm in accomplishments] 
        return domain_accomplishments
    else:
        accomplishments:Any = session.query(AccomplishmentORM).filter(
            AccomplishmentORM.status == "locked").filter(
                or_(
                AccomplishmentORM.name.ilike(f"%{search}%"),
                AccomplishmentORM.description.ilike(f"%{search}%")
        )).all()
...
def update_accomplishment(
    ...
    domain.difficulty = new_difficulty if new_difficulty is not None else domain.difficulty
...
def control_accomplishment_attribute_links(control:str, domain:Accomplishment,attribute: Attribute, load:int | None = None):
    ...
    if control == "link":
        ...
        association = AccomplishmentAttributeORM(accomplishment_id=accomplishment_orm.id, attribute_id=attribute_orm.id, load=load)
        ...
        accomplishment_orm=accomplishment_domain_to_orm(domain)
        attribute_orm=attribute_domain_to_orm(attribute)
        association=session.query(AccomplishmentAttributeORM).filter(and_(
            AccomplishmentAttributeORM.accomplishment_id == accomplishment_orm.id,
...
def control_accomplishment_profession_links(control:str, domain:Accomplishment,profession: Profession, load:int | None = None):
    ...
    if control == "link":
        ...
        association = AccomplishmentProfessionORM(accomplishment_id=accomplishment_orm.id, profession_id=profession_orm.id, load=load)
        session.add(association)
        session.commit()
        ...
        profession.accomplishments= [acc for acc in profession.accomplishments if acc.name != domain.name]
        return domain
    else:
        print("invalid control")

def control_accomplishment_mission_links(control:str, domain:Accomplishment,mission: Mission):
    ...
    if control == "link":
        ...
        association = MissionAccomplishmentORM(accomplishment_id=accomplishment_orm.id, mission_id=mission_orm.id)
        session.add(association)
        session.commit()
        ...
        mission.accomplishments= [acc for acc in mission.accomplishments if acc.name != domain.name]
        return domain
    else:
        print("invalid control")

def control_accomplishment_title_links(control:str, domain:Accomplishment,title: Title):
    ...
    if control == "link":
        ...
        association = AccomplishmentTitleORM(accomplishment_id=accomplishment_orm.id, title_id=title_orm.id)
        session.add(association)
        session.commit()
        ...
        title.accomplishments= [acc for acc in title.accomplishments if acc.name != domain.name]
        return domain
    else:
        print("invalid control")
    
def delete_accomplishment(domain:Accomplishment):
    from src.models.db.models import AccomplishmentAttributeORM, AccomplishmentProfessionORM, AccomplishmentTitleORM, MissionAccomplishmentORM
    orm = accomplishment_domain_to_orm(domain)
    target_id = orm.id
    session.query(AccomplishmentAttributeORM).filter(AccomplishmentAttributeORM.accomplishment_id == target_id).delete(synchronize_session=False)
    session.query(AccomplishmentProfessionORM).filter(AccomplishmentProfessionORM.accomplishment_id == target_id).delete(synchronize_session=False)
    session.query(AccomplishmentTitleORM).filter(AccomplishmentTitleORM.accomplishment_id == target_id).delete(synchronize_session=False)
    session.query(MissionAccomplishmentORM).filter(MissionAccomplishmentORM.accomplishment_id == target_id).delete(synchronize_session=False)
    session.delete(orm)
    session.commit()
    del domain
