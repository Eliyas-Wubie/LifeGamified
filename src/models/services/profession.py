...
def profession_domain_to_orm(domain: Profession, session: Session) -> ProfessionORM:
    prof: Any = session.query(ProfessionORM).filter(ProfessionORM.name == domain.name).first()
    return prof


def create_profession_orm(
    domain: Profession, cache: dict[int, ProfessionORM] | None = None
) -> ProfessionORM:
    if cache is None:
        cache = {}
    ...
    orm = ProfessionORM(name=domain.name, status=domain.status, points=domain.points)
    cache[key] = orm
    orm.sub_professions = [create_profession_orm(child, cache) for child in domain.sub_professions]
    return orm
...
def view_professions(search: str | None = None) -> list[Profession]:
    with SessionLocal() as session:
        if not search:
            ...
            return [profession_orm_to_domain(orm) for orm in professions]

        professions = (
            session.query(ProfessionORM)
            .filter(ProfessionORM.name.ilike(f"%{search}%"))
            .all()
        )
        return [profession_orm_to_domain(orm) for orm in professions]
...
def view_profession_tree():
    with SessionLocal() as session:
        professions = (
            session.query(ProfessionORM)
            .filter(ProfessionORM.parent == None)
            .options(joinedload(ProfessionORM.sub_professions))
            .all()
        )
        return professions
...
def update_profession(
    ...
    with SessionLocal() as session:
        orm: ProfessionORM = profession_domain_to_orm(domain, session)
        if not orm:
            orm = create_profession_orm(domain)
            session.add(orm)
        ...
        domain.parent = (
            profession_orm_to_domain(new_parent) if new_parent is not None else domain.parent
        )
...
def control_profession_compound_activity_link(
    control: str,
    domain: Profession,
    ...
    with SessionLocal() as session:
        if control == "link":
            profession_orm = profession_domain_to_orm(domain, session)
        ...
        if control == "unlink":
            ...
            compound_activity.professions = [
                prof
                for prof in compound_activity.professions
                if prof.name != domain.name
            ]
            return domain
        print("invalid control")


def control_profession_accomplishment_link(
    control: str,
    domain: Profession,
    ...
    with SessionLocal() as session:
        ...
        if control == "unlink":
            ...
            accomplishment.professions = [
                mission
                for mission in accomplishment.professions
...
def control_profession_mission_link(
    control: str, domain: Profession, mission: "Mission", load: int | None = None
):
    ...
    with SessionLocal() as session:
        ...
        if control == "unlink":
            ...
            mission.professions = [
                prof for prof in mission.professions if prof.name != domain.name
            ]
            return domain
        print("invalid control")
...
def delete_profession(domain: Profession):
    ...
    with SessionLocal() as session:
        orm = profession_domain_to_orm(domain, session)
        target_id = orm.id
        session.query(CompoundActivityProfessionORM).filter(
            CompoundActivityProfessionORM.profession_id == target_id
        ).delete(synchronize_session=False)
        session.query(AccomplishmentProfessionORM).filter(
            AccomplishmentProfessionORM.profession_id == target_id
        ).delete(synchronize_session=False)
        session.query(MissionProfessionORM).filter(
            MissionProfessionORM.profession_id == target_id
        ).delete(synchronize_session=False)
        session.delete(orm)
        session.commit()
    del domain
