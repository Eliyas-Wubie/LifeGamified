...
def title_domain_to_orm(domain: Title, session: Session) -> TitleORM:
    title: Any = session.query(TitleORM).filter(TitleORM.name == domain.name).first()
    return title


def create_title_orm(domain: Title) -> TitleORM:
    return TitleORM(name=domain.name, description=domain.description)
...
def view_owned_titles(search: str | None = None) -> list[Title]:
    with SessionLocal() as session:
        if not search:
            ...
            return [title_orm_to_domain(orm) for orm in titles]

        titles = (
            session.query(TitleORM)
            .filter(TitleORM.status == "unlocked")
            .filter(
                or_(
...
def update_title(
    ...
    with SessionLocal() as session:
        orm: TitleORM = title_domain_to_orm(domain, session)
        if not orm:
            orm = create_title_orm(domain)
...
def control_title_accomplishment_link(
    control: str, domain: Title, accomplishment: "Accomplishment"
):
    ...
    with SessionLocal() as session:
        if control == "link":
            title_orm = title_domain_to_orm(domain, session)
            accomplishment_orm = accomplishment_domain_to_orm(accomplishment, session)
            association = AccomplishmentTitleORM(
                accomplishment_id=title_orm.id, mission_id=accomplishment_orm.id
            )
        ...
        if control == "unlink":
            ...
            accomplishment.titles = [
                t for t in accomplishment.titles if t.name != domain.name
            ]
            return domain
...
def delete_title(domain: Title):
    from src.models.db.models import AccomplishmentTitleORM, StatusTitleORM

    with SessionLocal() as session:
        orm = title_domain_to_orm(domain, session)
        target_id = orm.id
        session.query(StatusTitleORM).filter(
            StatusTitleORM.title_id == target_id
        ).delete(synchronize_session=False)
        session.query(AccomplishmentTitleORM).filter(
            AccomplishmentTitleORM.title_id == target_id
        ).delete(synchronize_session=False)
        session.delete(orm)
        session.commit()
    del domain
